...
if __name__ == '__main__':
    ...
    root_dir = 'your/dataset/original_images'
    out_dir = 'your/out/crop/dir'
    import cv2
    ...
    for face_path in files:
        ...
        res_np = draw_landmarks(face_lm_68, face_img_bgr.copy())
        # color_texture landmark image
        # cv2.imwrite(os.path.join(out_dir, face_path), res_np)

        face_img_pil, face_lm_68 = recreate_aligned_images(face_img_rgb, face_lm_68, output_size=256)

        img_np = np.array(face_img_pil)
        res_np = img_np
        # res_np = draw_landmarks(face_lm_68, img_np, font_size=0.3)
        cv2.imwrite(os.path.join(out_dir, face_path), cv2.cvtColor(res_np, cv2.COLOR_RGB2BGR))
